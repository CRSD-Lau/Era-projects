...
function onMobDeathEx(mob, player, isKiller, isWeaponSkillKill)
	...
	if (loc == 51 or (loc >= 52 and loc <= 56) or (loc >= 61 and loc <= 63) or loc == 65 or loc == 68 or loc == 72 or loc == 79 ) then
		...
		elseif (mob:getMainLvl() >= 61 and mob:getMainLvl() <= 65) then
		    player:addCurrency("zeni_point", math.random(5,9));
		elseif (mob:getMainLvl() >= 66 and mob:getMainLvl() <= 70) then
		    player:addCurrency("zeni_point", math.random(7,13));
		elseif (mob:getMainLvl() >= 71 and mob:getMainLvl() <= 75) then
		    player:addCurrency("zeni_point", math.random(9,15));
		elseif (mob:getMainLvl() >= 76 and mob:getMainLvl() <= 80) then
